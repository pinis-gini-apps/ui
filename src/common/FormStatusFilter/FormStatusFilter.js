...
import './formStatusFilter.scss'

const FormStatusFilter = ({ label, name }) => {
  const { input } = useField(name)
  const [isDropDownMenuOpen, setIsDropDownMenuOpen] = useState(false)
  const [tagFilter, setTagFilter] = useState(input.value)
  const [tagOptions] = useState(generateStatusFilter(true))
  ...
  //   let newTagOptions = generateStatusFilter
  //   let pageTagList = []
  //
  //   if (filtersStore.tagOptions?.length > 0) {
  //     const defaultOptionsTags = generateStatusFilter.map(option => option.id)
  //     pageTagList = [...generateStatusFilter]
  //     let contentTagList = []
  //
  //     if (content) {
  //       contentTagList = uniq(content.map(contentItem => contentItem.tag))
  //       console.log(contentTagList)
  //     }
  //
  //     newTagOptions = [
    ...
    event => {
      ...
      if (
        ...
        if (tagFilter.length <= 0) {
          ...
          setTagFilter(generateStatusFilter.find(tag => tag.id === TAG_FILTER_LATEST).label)
        } else {
          const tag = generateStatusFilter.find(tag => tag.id === tagFilter)
